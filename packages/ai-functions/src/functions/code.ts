...
async function codeCore(prompt: string, options: CodeOptions = {}): Promise<string> {
  ...
  const { language = 'typescript', framework, style = 'functional' } = options
...
export const code = createUnifiedFunction<Promise<string>>((prompt: string, options: Record<string, any>) => {
