/**
 * Event system for AI workflows
 * Extracted from MDXE CLI event system
 */
...
/**
 * Enhanced event context that includes AI functions
 */
export interface EnhancedEventContext extends MutableEventContext {
  ai?: any
  list?: any
  research?: any
  extract?: any
...
export class EventRegistry {
   ...
   */
  on(event: string, callback: (data: any, context?: EnhancedEventContext) => Promise<any> | any, timeout?: number) {
    ...
    if (options.parallel && handlers.length > 0) {
      try {
        const handlerPromises = handlers.map(async (handler, index) => {
          try {
            const result = await this.executeHandlerWithTimeout(handler, data, enhancedContext, handler.timeout || options.timeout)
